...
public class ServisaAuto {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				try {
					...
					Administrator admin = new Administrator(id, obrisan, ime, prezime, jmbg, pol, adresa, brTelefona,
							username, password, plata);
	...
	public void sacuvajAdmin() {
		try {
			...
			String stringa = "";
			for (Administrator admin : admini) {
				stringa += admin.getId() + "|" + admin.isObrisan() + "|" + admin.getIme() + "|" + admin.getPrezime()
						+ "|" + admin.getJmbg() + "|" + admin.getPol() + "|" + admin.getAdresa() + "|"
						+ admin.getBrTelefona() + "|" + admin.getUsername() + "|" + admin.getPassword() + "|"
						+ admin.getPlata() + "\n";
			}
			br.write(stringa);
			br.close();
	...
	public void sacuvajAdmin(Administrator admin) {
		try {
			...
			stringa += admin.getId() + "|" + admin.isObrisan() + "|" + admin.getIme() + "|" + admin.getPrezime() + "|"
					+ admin.getJmbg() + "|" + admin.getPol() + "|" + admin.getAdresa() + "|" + admin.getBrTelefona()
					+ "|" + admin.getUsername() + "|" + admin.getPassword() + "|" + admin.getPlata() + "\n";
	...
	public void sacuvajDeo(ServDeo deo) {
		try {
			...
			stringa += deo.getId() + "|" + deo.isObrisan() + "|" + deo.getMarkaVoz() + "|" + deo.getModelVoz() + "|"
					+ deo.getNazivDela() + "|" + deo.getCena() + "\n";
	...
	public void sacuvajServis() {
		try {
			...
			String stringa = "";
			for (ServisAutomobila servis : servisi) {
				stringa += servis.getId() + "|" + servis.isObrisan() + "|" + servis.getMarka() + "|" + servis.getModel()
						+ "|" + servis.getServ().getId() + "|" + servis.getTermin() + "|" + servis.getOpis() + "|"
						+ servis.getStatus() + "|";
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				try {
					...
					Serviser serviser = new Serviser(id, obrisan, ime, prezime, jmbg, pol, adresa, brTelefona, username,
							password, plata, spec);
	...
	public void sacuvajServisera() {
		try {
			...
			String stringa = "";
			for (Serviser serviser : serv) {
				stringa += serviser.getId() + "|" + serviser.isObrisan() + "|" + serviser.getIme() + "|"
						+ serviser.getPrezime() + "|" + serviser.getJmbg() + "|" + serviser.getPol() + "|"
						+ serviser.getAdresa() + "|" + serviser.getBrTelefona() + "|" + serviser.getUsername() + "|"
						+ serviser.getPassword() + "|" + serviser.getPlata() + "|" + serviser.getSpec() + "\n";
			}
			br.write(stringa);
			br.close();
		} catch (Exception e) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				try {
					...
					Musterija must = new Musterija(id, obrisan, ime, prezime, jmbg, pol, adresa, brTelefona, username,
							password, nagrbodovi);
	...
	public void ucitajAutomobile() {
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				musterija1.setId(Integer.parseInt(split[2]));
				Musterija musterija2 = servis12.nadjiMusteriju(Integer.parseInt(split[2]));
				Musterija musterija = musterija2 != null ? musterija2 : musterija1;
				...
				Automobil auto = new Automobil(id, obrisan, musterija, marka, model, godproiz, zaprMotora, snagaMotora,
						vrstagor);
				auti.add(auto);
	...
	public void sacuvajMusteriju() {
		try {
			...
			String stringa = "";
			for (Musterija must : musterije) {
				stringa += must.getId() + "|" + must.isObrisan() + "|" + must.getIme() + "|" + must.getPrezime() + "|"
						+ must.getJmbg() + "|" + must.getPol() + "|" + must.getAdresa() + "|" + must.getBrTelefona()
						+ "|" + must.getUsername() + "|" + must.getPassword() + "|" + must.getNagrBodovi() + "\n";
			}
			br.write(stringa);
			br.close();
		} catch (Exception e) {
		...
		/*
		 * int id,boolean obrisan,String ime, String prezime, long jmbg, String pol,
		 * String adresa, String brTelefona, String username, String password, short
		 * nagrBodovi
		 */
	...
	public void sacuvajAuto() {
		try {
			...
			String stringa = "";
			for (Automobil auto : auti) {
				stringa += auto.getId() + "|" + auto.isObrisan() + "|" + auto.getVlasnik().getId() + "|"
						+ auto.getMarka() + "|" + auto.getModel() + "|" + auto.getGodProiz() + "|"
						+ auto.getZaprMotora() + "|" + auto.getSnagaMotora() + "|" + auto.getVrstaGoriva() + "\n";
			}
			br.write(stringa);
			br.close();
		} catch (Exception e) {
	...
	public void sacuvajAuto(Automobil auto) {
		try {
			...
			stringa += auto.getId() + "|" + auto.isObrisan() + "|" + auto.getVlasnik().getId() + "|" + auto.getMarka()
					+ "|" + auto.getModel() + "|" + auto.getGodProiz() + "|" + auto.getZaprMotora() + "|"
					+ auto.getSnagaMotora() + "|" + auto.getVrstaGoriva() + "\n";
	...
	public boolean LoginMusterija(String username, String pass) {
		for (Musterija m : musterije) {
			if (m.getUsername().equals(username) && m.getPassword().equals(pass)) {
				LoginWin.currentUser = m;
				return true;
	...
	/*
	 * int id,Musterija vlasnik, MarkaVozila marka, ModeliAuto model, short
	 * godProiz, int zaprMotora, int snagaMotora, VrstaGoriva vrstaGor,boolean
	 * obrisan
	 */
